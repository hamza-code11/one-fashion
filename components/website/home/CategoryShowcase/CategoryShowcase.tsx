...
// components/website/home/CategoryShowcase/CategoryShowcase.tsx
'use client';
...
const DEFAULT_CENTER = Math.floor(VISIBLE_CARDS / 2); // slot 2
...
const CategoryShowcase: React.FC = () => {
  // By default the CENTER card is expanded (matches reference image).
  // On hover of any other card, that card expands and the center collapses.
  const [activeSlot, setActiveSlot] = useState<number | null>(DEFAULT_CENTER);

  const visible = categories.slice(0, VISIBLE_CARDS);
    ...
    <section className={styles.section}>
      <div className={styles.container}>
        ...
        <div
          ...
          onMouseLeave={() => setActiveSlot(DEFAULT_CENTER)}
        >
          {visible.map((cat, slot) => {
            ...
            return (
              <article
                ...
                tabIndex={0}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' || e.key === ' ') setActiveSlot(slot);
                  if (e.key === 'Escape') setActiveSlot(DEFAULT_CENTER);
                }}
              ...
              >
                ...
                >
                  ...
                  <span className={styles.cardVerticalTitle}>{cat.title}</span>

                  {/* rich content — shown on expanded card */}
                  {isActive && (
                    <div className={styles.cardContent}>
                      <span className={styles.cardEyebrow}>Collections</span>
                      <h3 className={styles.cardTitle}>{cat.title}</h3>
                      <Link href="" className={styles.cardLink}>
                        Shop Now
                        <span aria-hidden="true">↗</span>
